...
The best way to deal with missing values in a data set is to understand why those values went missing. This option may not be able in many cases, so, we can do either one of the two options below. 

1. Drop the entire row with missing values, however, just because one value is missing, it doesn't mean that other values in the row are faulty. If we have a really faulty data set, by the time we end up dropping missing values, the entire data set is gone. 
